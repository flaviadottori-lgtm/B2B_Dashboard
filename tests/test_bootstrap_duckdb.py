...
def test_bootstrap_duckdb_contract():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        df = pd.DataFrame({
            "ano": [2022], "quarter": ["Q1"], "gender": ["M"], "age_group": ["25-29"], "value": [1000.0], "uf_code": ["11"]
        })
        df.to_parquet(parquet_path)
        con.execute(f"CREATE OR REPLACE TABLE caged_state_sector_year (year INTEGER, cod_uf INTEGER, sector VARCHAR, value DOUBLE, uf_code VARCHAR);")
        con.execute(f"INSERT INTO caged_state_sector_year VALUES (2022, 11, 'Agro', 500.0, '11');")
        # Simula ingestão
        con.execute(f"CREATE OR REPLACE TABLE pnad_uf_quarter_gender_age AS SELECT * FROM read_parquet('{str(parquet_path)}')")
        ensure_dim_uf(con)
        ensure_views(con)
        ensure_dim_uf(con)
        ensure_views(con)
