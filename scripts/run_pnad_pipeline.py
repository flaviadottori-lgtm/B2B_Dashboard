...
def main():
    parser = argparse.ArgumentParser(
        description='Extract PNAD Contínua data from BigQuery'
    )
    parser.add_argument(
        '--min-year',
        type=int,
    ...
    parser.add_argument(
        '--no-save',
        action='store_true',
        help='Only extract to DataFrame, do not save parquet'
    )
    parser.add_argument(
        '--project',
        type=str,
        default='b2b-opportunity-engine',
    ...
    if args.output is None and not args.no_save:
        args.output = project_root / 'data' / 'marts' / 'pnad' / 'pnad_uf_quarter_gender_age.parquet'
