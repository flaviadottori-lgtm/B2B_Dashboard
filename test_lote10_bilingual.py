...
def test_app_uses_i18n_not_hardcoded():
    ...
    assert 'T["map_col_index"]' in map_section or "T['map_col_index']" in map_section, \
        "map_col_index not used via T[] in map section"
    
    assert 'T["map_title"]' in map_section or "T['map_title']" in map_section, \
...
def test_sample_bilingual_output():
    ...
    for lang in ["pt", "en"]:
        ...
        print(f"  Columns: {T['map_col_rank']}, {T['map_col_state']}, {T['map_col_region']}, {T['map_col_index']}")
